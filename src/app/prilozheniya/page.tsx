...
// lang приходит от языкового зеркала ([lang]/prilozheniya); без него — русская версия.
export default function AppsPage({ lang }: { lang?: string } = {}) {
  ...
  const apps = getApps()
    ...
    .map((a) => (loc ? translateData(a, loc).data : a));
  ...
  return (
    <>
      ...
      <div className="bg-brand text-white" data-a11y-surface="brand">
        <div className="mx-auto max-w-[1146px] px-10 py-8 box-border max-[768px]:px-5">
          ...
          <p className="m-0 max-w-[720px] font-ui text-[18px] text-white/85">
            {S("Сервисы и информационные системы университета. Выберите, кто вы, — список подстроится. Часть систем служебные и требуют учётной записи.")}
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1146px] w-full px-10 pt-9 pb-16 box-border grid grid-cols-[250px_1fr] gap-10 max-[900px]:grid-cols-1 max-[768px]:px-5 font-ui">
        ...
        {loc && <TranslationNotice lang={loc} originalHref="/prilozheniya" />}
        <AppsRegistry apps={apps} ui={uiStrings(APPS_UI, lang)} />
        </main>
      </div>
    </>
  );
