...
import RuPage, { generateStaticParams as ruParams } from "@/app/app-launcher/[instance]/page";
...
export async function generateMetadata({
  ...
  params: Promise<{ lang: string; instance: string }>;
}): Promise<Metadata> {
  const { lang, instance } = await params;
  if (!isTargetLocale(lang)) return {};
  const ru = `/app-launcher/${instance}`;
...
export default async function Mirror({
  ...
  return <RuPage params={Promise.resolve({ instance })} />;
}
